...
export default function Page({
  ...
  const handleAddToCart = async () => {
    ...
    if (alreadyExistThisProductInCart)
      return toast("Produto já existe na sacola", {
        action: {
          label: "Ir a sacola",
    ...
    const priceWithDiscount = productDetails.valuePromotionInPercent
      ? productDetails.price / 100 -
        (productDetails.price / 100) *
          (productDetails.valuePromotionInPercent / 100)
      : productDetails.price / 100;
    const newCartItem = {
    ...
    toast("Produto adicionado!", {
      description: `${productDetails.name}, ${amount} ${
        amount === 1 ? "unidade" : "unidades"
      }`,
      action: {
        label: "Ir a sacola",
        onClick: () => setOpenCart(true),
  ...
  return loading ? (
    ...
    <div className="w-full min-h-screen lg:bg-gray-200 pb-8">
          ...
          <Sheet open={openCart} onOpenChange={setOpenCart}>
            <SheetTrigger>
              <div
                className={`${
                  cart.length > 0
                    ? "bg-red-500 text-white flex w-4 h-4 rounded-full  float-right relative z-50 full justify-center items-center"
                    : "hidden"
                }`}
              >
                1
      ...
      <div
        ...
        className="w-full h-[60vh] md:hidden bg-center justify-end flex flex-col items-center bg-cover bg-no-repeat"
      >
        <div className="w-full h-full -z-10 bg-gradient-to-b from-[rgba(0,0,0,0.0)] to-[rgba(0,0,0,0.2)]"></div>
      </div>
      {/* Desktop */}
      <div className="hidden  pt-32 md:block">
        ...
        <div className="md:flex w-[80vw] xl:max-w-[1300px] mx-auto h-[75vh] px-10 hidden bg-white py-6 rounded-t-3xl shadow-md justify-between  ">
          <div
            style={{ backgroundImage: `url(${productDetails.imageUrl})` }}
            className="w-[35%] h-full shadow-lg rounded-lg flex justify-center items-end bg-cover bg-center bg-no-repeat"
          >
            <div className=" justify-between w-14 flex mb-5 items-center"></div>
          </div>
          <div className="w-[60%]">
            <h3 className="font-semibold text-primary mt-5">Variação</h3>
              ...
              {variants.length === 0 ? (
                ...
                variants?.map((variant: any, index: number) => (
                  <button
                    ...
                    key={index}
                    className={`h-10  flex justify-center rounded  items-center ${
                      variant.amount === 0
                        ? "bg-opacity-60 text-red-500"
                        : "cursor-pointer"
                  ...
                  >
                    {variant.name}
                    {variant.amount === 0 && (
                      <p className="text-red-500 ml-2">Esgotado!</p>
                    )}
                  </button>
                ))
              )}
            ...
            <div className="w-full mt-5">
              <div className="">
                <div className="w-f">
                  ...
                  <textarea className="w-[100%] h-24  rounded-lg border-[0.5px] border-zinc-300 px-4 py-2"></textarea>
                </div>
                <Button
            ...
            <div className="flex items-end mt-12 justify-between w-3/4">
              <div>
                ...
                <div className="flex items-center gap-2 text-2xl">
                  <button
                    ...
                    className="text-2xl text-white w-10 h-10 flex justify-center items-center bg-primary rounded-lg"
                  >
                    -
                  </button>
                  <input
              ...
              <div>
                ...
                <Button
                  ...
                  className="gap-4 shadow bg-primary mt-5 -z-30 h-10 px-10"
                >
        ...
        <div className="w-[80vw]  xl:max-w-[1300px] mx-auto bg-white p-6 rounded-b-3xl">
          ...
          <div className="grid gap-5 grid-cols-4">
            {relationedProducts?.map((product: any, index: number) => {
              ...
              const priceWithDiscount = product.valuePromotionInPercent
                ? product.price / 100 -
                  (product.price / 100) *
                    (product.valuePromotionInPercent / 100)
                : product.price / 100;
              return (
                <ProductCard
                  key={index}
      ...
      <div className="w-full h-40 md:hidden pt-8  bg-white md:rounded-none px-4 rounded-t-3xl md:mt-0 -mt-16">
        ...
        <h3 className="font-semibold md:hidden text-primary mt-5">Variação</h3>
        ...
        <div className="flex items-end w-full mt-5 justify-between">
          <div>
            ...
            <div className="flex mt-5 items-center gap-2">
              ...
              <input
                ...
                className="w-10 border-[0.5px] text-center border-zinc-300 h-10"
                type="text"
              />
              <Button
                className="text-2xl text-white w-10 h-10 flex justify-center items-center bg-primary rounded-lg"
         ...
         text-primary font-bold my-5 gap-2"
          >
            <h3 className="text-xl font-medium">Total R$</h3>
        ...
        <h3 className="font-semibold text-primary mt-5">Descrição</h3>
      ...
      <div
        className={`fixed ${
          loading ? "hidden" : ""
        } md:hidden h-20 pb-4 bg-white left-0 z-50 w-full justify-center flex items-center bottom-0`}
      >
        <Button
          onClick={handleAddToCart}
          disabled={!selectedVariant}
          className="w-[90%] bg-primary gap-2 h-12 mt-5"
        >
